# 1.cluster.py
# t분포의 자유도로 soft assignment식 구현
# 2. dec.py
# 오토인코더의 인코터 부분으로 soft assignment식 사용 -> cluster assignment 계산
# 3. utils.py
# 1) cluster_accuracy
# ㅁ 클러스터링 정확도 계산수행하기 위한 함수
# ㅁ linear_sum_assignment=Hugrian Algorithm -> 클러스터링 군집번호를 라벨과 맞게 재정렬하는 과정수행(열, 행)
# 2) target_distribution
# ㅁ 라벨링되지않은 데이터셋에 초기 classfier를 사용해 라벨데이터를 만드는 방식
# ㅁ KL-divergence사용. 계속 클러스터를 재정의하며 soft assginment를 target distribution에 매칭
# ㅁ p_ij를 구하는 식을 리턴. i번째 데이터가 j번쨰 cluster에 속할 확률
...
df = pd.read_csv('../naver_preprocessing_tokenizing_review.csv')
...
for idx, text in enumerate(df['preprocessed_reivew']):
    try:
        ...
        if tokenizer == []:
            ...
            tokenized.append([tokenizer, idx+1])
    except:
        continue
print(tokenized)
