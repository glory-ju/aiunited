...
def get_dataset(model, reviews, size):
    dataset = list()

    for token_list in tqdm(reviews):
        dataset.append(get_features(model, token_list, size))
    reviewFeatureVecs = np.stack(dataset)

    return reviewFeatureVecs

def load_word2vec(df, **param):
    # df = df[df['tokenized_review'].str.len() != 0] # tokenized_review 빈 리스트 제거
    tokenized_review = df['tokenized_review']
    model = Word2Vec.load('weights/word2vec.bin')
...
def load_data(dataset_name, data, **param):
    if dataset_name == 'word2vec':
        return load_word2vec(data, **param)
