...
def load_kobert(df):
    ...
    data_x = BERTDataset(df, '0', tok, 128, True, False)
...
def load_data(dataset_name, data, **param):
    ...
    if dataset_name == 'doc2vec':
        # df = pd.read_csv(r'C:/Users/quzmi/PycharmProjects/aiunited/embedding/naver_tokened.csv')
        # df['tokenized_review'] = df.tokenized_review.apply(lambda x: x[1:-1].split(','))
        # df['tokenized_review'] = np.where(pd.notnull(df['tokenized_review']) == True, df['tokenized_review'],
        #                                   max(df['tokenized_review']))
        # df.dropna(inplace=True)
        return load_doc2vec(data, **param)
    elif dataset_name == 'kobert':
