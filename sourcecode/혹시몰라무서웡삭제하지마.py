...
def to_csv(i, idx):
    ...
    dataset.to_csv('naver_store_info2.csv', encoding='UTF-8', index=False)
...
for i in range(100):
    df['new_name'] = df['store_name'][i] + ' ' + df['store_addr'][i].split(' ')[2]
    ...
    try:
        try:
            try:
                csv_addr = df['store_addr'][i]

                for idx in range(len(load['result']['place']['list'])):
                    store_addr = load['result']['place']['list'][idx]['address']

                    if csv_addr[:15] in store_addr:
                        store_info(idx)
                        to_csv(i, idx)
                        break
                    elif csv_addr[:12] in store_addr:
                        store_info(idx)
                        to_csv(i, idx)
                        break
                    elif csv_addr[:10] in store_addr:
                        store_info(idx)
                        to_csv(i, idx)
                        break
                    elif csv_addr[:7] in store_addr:
                        store_info(idx)
                        to_csv(i, idx)
                        break
            ...
            for idx in range(len(load['result']['place']['list'])):
                ...
                if csv_addr[:12] in store_addr:
                    store_info(idx)
                    to_csv(i, idx)
                    break
                elif csv_addr[:15] in store_addr:
                    store_info(idx)
                    to_csv(i, idx)
                    break
    except:
