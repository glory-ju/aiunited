...
info_df = pd.read_csv('storeInfo_1.csv')
info_df['new_score'] = ''
info_df['review_cnt'] = ''
...
for store_id in tqdm(info_df['store_id']):
    ...
    review_cnt = len(review_df['new_score'])
    new_score = np.round(sum(review_df['new_score']) / review_cnt, 2)
...
for store_id in info_df['store_id']:
    if store_id in no_store_review_lst:
        info_df.loc[(df.store_id == store_id), 'new_score'] = ''
        info_df.loc[(df.store_id == store_id), 'review_cnt'] = 0
